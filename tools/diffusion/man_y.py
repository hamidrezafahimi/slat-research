...
def main(argv=None):
    ...
    ref_img_orig_ = cv2.imread(str(ref_path), cv2.IMREAD_GRAYSCALE)
    ref_img_orig = ref_img_orig_.T
    ref_img_orig = cv2.cvtColor(ref_img_orig, cv2.COLOR_GRAY2BGR)
    ...
    while True:
        ...
        arr = patt_gray_img + loaded_pattern.astype(np.int32) - 127
