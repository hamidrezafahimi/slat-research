...
def make_pattern_gray(ys):
    ...
    intens = (h_from_bottom / (CANVAS_H - 1) * 255).astype(np.uint8)
...
def main(argv=None):
    ...
    # ↓ spline state ------------------------------------------------------
    if args.spline:
        C = np.loadtxt(args.spline, delimiter=',').astype(np.float32)
        ctrl_pts = []
        for k in range(C.shape[0]):
            ctrl_pts.append({'x': int(C[k,0]), 'y': int(C[k,1]), 'color': (int(C[k,2]), int(C[k,3]), int(C[k,4]))})
    else:
        ctrl_pts = [
    ...
    while True:
        ...
        overlay = cv2.resize(ref_img_orig, (ref_w, ref_h), interpolation=cv2.INTER_LINEAR)
        ...
        cv2.imshow("pattern", patt_gray_img)
        mask_bool = cv2.cvtColor(overlay, cv2.COLOR_BGR2GRAY) > patt_gray_img
        ...
        if key == ord('s'):
            ...
            C = []
            for c in ctrl_pts:
                C.append([int(c['x']), int(c['y']), int(c['color'][0]), int(c['color'][1]), int(c['color'][2])])
            np.savetxt('spline_data.txt', np.array(C, dtype=np.float32), delimiter=',')
