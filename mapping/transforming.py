...
def transform_depth(depth_image, bg_image, pitch, log_dir="logs"):
    ...
    # np.savetxt(os.path.join(log_dir, "depth_image.csv"), depth_f, delimiter=",", fmt="%.4f")
    # np.savetxt(os.path.join(log_dir, "bg_image.csv"), bg_f, delimiter=",", fmt="%.4f")
    # np.savetxt(os.path.join(log_dir, "gep_f.csv"), gep_f, delimiter=",", fmt="%.4f")
    # np.savetxt(os.path.join(log_dir, "trns.csv"), trns, delimiter=",", fmt="%.4f")
    # np.savetxt(os.path.join(log_dir, "final_f.csv"), final_f, delimiter=",", fmt="%.4f")
...
def plot_point_clouds_surface(pc1=None, pc2=None, pc3=None, pc4=None, step=20):
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')

    # Define colormaps and labels for up to 4 clouds
    colormaps = ['viridis', 'plasma', 'coolwarm', 'cividis']
    labels = ['PointCloud 1', 'PointCloud 2', 'PointCloud 3', 'PointCloud 4']
    ...
    for i, pc in enumerate(point_clouds):
        if pc is not None:
            ...
            # Plot as surface
            surf = ax.plot_surface(X, Y, Z, cmap=colormaps[i], linewidth=0, antialiased=False, alpha=0.8)
            fig.colorbar(surf, ax=ax, shrink=0.5, aspect=10, label=labels[i])

    # Plot the origin
    ax.scatter(0, 0, 0, color='red', label='Origin', s=50)
...
def main():
    if len(sys.argv) < 4:
        print("Usage: python script.py depth_image.jpg background.jpg converted_data.txt")
        sys.exit(1)

    depth_path = sys.argv[1]
    bg_path = sys.argv[2]
    tilt_file = sys.argv[3]

    # Extract base filenames without extensions
    depth_base = os.path.splitext(os.path.basename(depth_path))[0]
    bg_base = os.path.splitext(os.path.basename(bg_path))[0]

    # Check if base filenames match
    if depth_base != bg_base:
        raise ValueError("Depth and background image base filenames must match (ignoring extensions).")

    # Read tilt/converted data file
    with open(tilt_file, "r") as file:
        tlines = file.readlines()

    # Filter matching entries
    matching_lines = [line.strip() for line in tlines if line.startswith(depth_base + ".jpg,")]

    if len(matching_lines) != 1:
        raise ValueError(f"Expected exactly one match for {depth_base} in {tilt_file}, found {len(matching_lines)}.")

    # Parse pitch from the matched line
    parts = matching_lines[0].split(',')
    pitch = float(parts[4])

    # Load depth image
    depth_image = np.loadtxt(depth_path, delimiter=',', dtype=np.float32)
    depth_image_eq = 255 - cv2.imread("00000001.jpg", cv2.IMREAD_GRAYSCALE)
    ...
    if bg_image is None:
        raise IOError(f"Could not load {bg_path}. Check the file path.")

    # Ensure image dimensions match
    if depth_image.shape != bg_image.shape:
    ...
    bg_scaled = bg_image.astype(np.float32) * ((np.max(depth_image_scaled) - np.min(depth_image_scaled)) / 255.0) +\
          np.min(depth_image_scaled)
    # bg_scaled = bg_scaled.astype(np.uint8)
    # cv2.imwrite("fasdg.jpg", bg_scaled)
    fimg, gep_depth = transform_depth(depth_image_scaled, bg_scaled, pitch)
    # cv2.imshow("asdf", depth_image_scaled.astype(np.uint8))
    # cv2.imwrite("asdf.jpg", depth_image_scaled.astype(np.uint8))
    # np.savetxt(os.path.join("depth_image_scaled.csv"), depth_image_scaled, delimiter=",", fmt="%.4f")
    # print(np.min(bg_image))
    # print(np.max(bg_image))
    # print(np.min(depth_image))
    # print(np.max(depth_image))
    # print(np.min(depth_image_eq))
    # print(np.max(depth_image_eq))
    # print(np.min(depth_image_scaled))
    # print(np.max(depth_image_scaled))


    # cv2.waitKey()
    # # fimg, gep_depth = rescale_depth(depth_image, bg_image, pitch)
    # # fimg, gep_depth = move_depth(depth_image, bg_image, pitch)
    # # cv2.imshow("sdf", gep_depth)
    # cv2.waitKey()
    # print(gep_depth.shape)

    bg_pc = depthImage2pointCloud(bg_image, pitch_rad=pitch, horizontal_fov=66)
    bgs_pc = depthImage2pointCloud(bg_scaled, pitch_rad=pitch, horizontal_fov=66)
    depth_pc = depthImage2pointCloud(depth_image_scaled, pitch_rad=pitch, horizontal_fov=66)
    deq_pc = depthImage2pointCloud(depth_image_eq, pitch_rad=pitch, horizontal_fov=66)
    ...
    # fimg_pc_new = push_points_behind_min_distance((0,0,0), 90, gep_pc, fimg_pc)
    # # print(gep_pc)

    # plot_point_cloud(bg_pc, step=10)
    # plot_point_cloud(gep_pc, step=10)
    # plot_point_cloud(depth_pc, step=10)
    plot_point_cloud(fimg_pc, step=1)
    # plot_point_cloud_2(fimg_pc_new, step=1)

    # plot_point_clouds_surface(bgs_pc, depth_pc)
    # plot_point_clouds_surface(bg_pc, deq_pc)
