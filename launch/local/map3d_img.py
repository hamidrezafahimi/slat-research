...
def main():
    ...
    cfg = Mapper3DConfig(ref_mode=RefusionMode.Drop,
                         vis = True,
                         plot = True,
                         color_mode = 'image',   # Options: 'image', 'proximity', 'constant', 'none'
                         backend = 'open3d' ,       # Options: 'open3d', 'matplotlib'
                         hfov_deg = 80.0
                         )
    ...
    mapper = Mapper3D(cfg)
    mapper.process(metric_depth=metric_depth, metric_bg=metric_bg, pose=p, color_img=color_img)
